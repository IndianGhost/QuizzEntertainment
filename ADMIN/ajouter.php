...
<div class="container">
    <div class="jumbotron">
        ...
        <p class="lead">&laquo;<em>Remplir</em> les tables de votre <strong>base de données</strong> sans avoir à
            toucher le minimum du code&raquo;<br>Bonne Continuation!</p>
    </div>
            ...
            <?php if (isset($_GET['ajpub']) OR isset($_GET['fold'])) {
                if ($_GET['ajpub'] == 1) {
                    ...
                    <div class="alert alert-warning" role="alert">
                        <strong>N.B:</strong> Un dossier nommé '<?php echo $_GET['fold']; ?>' au chemin suivant
                        'CONTROLEUR/resultat/' a été crée pour que vous puissiez associer des résultats à cette nouvelle
                        publication.
                    </div>
                <?php }//Fin if ==1
            ...
            <form action="ajout_pub.php" method="post" enctype="multipart/form-data">
                <fieldset>
                    ...
                    <div class="row labeldiv">
                        <label for="titre_publication" class="col-xs-6">Titre de la publication :</label><input
                            type="text" name="titre_publication" id="titre_publication" class="col-xs-5 col-xs-offset-1"
                            required/>
                    </div>

                    <div class="row labeldiv">
                        <label for="description_publication" class="col-xs-6">Description de la publication
                            :</label><input type="text" name="description_publication" id="description_publication"
                                            class="col-xs-5 col-xs-offset-1" required/>
                    </div>
                    ...
                    <div class="row labeldiv">
                        ...
                        <select name="id_publication" class="col-xs-8 col-xs-offset-1" required>
                            <?php while ($donnees = $reponse->fetch()) { ?>
                                <option
                                    value="<?php bye_XSS($donnees['id_publication']); ?>"><?php bye_XSS($donnees['titre_publication']); ?></option>
                            <?php }//FinWhile
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <p class="lead">La taille des image ne doit pas dépasser 8Mo et leurs extensions doivent être (.jpg). Il
                faut que la résolution soit (de préférence) 717 x 475 afin d'atteindre un affichage parfait.</p>
        </div>
    </div>
</div>
<div class="container">
