...
<div class="container">
    ...
    <div class="jumbotron">
        ...
        <p class="lead">&laquo;<em>Supprimer</em> n'importe quelle tables de votre <strong>base de données</strong> sans
            avoir à toucher le minimum du code&raquo;<br>Bonne Continuation!</p>
    </div>
    <div class="row">
        <?php if (isset($_GET['delpub'])) {
            if ($_GET['delpub'] == 1) {
                ?>
                <div class="alert alert-success" role="alert">
                    <strong>Suppression effectuée!</strong> Publication supprimée avec succès.
                </div>
            <?php } else if ($_GET['delpub'] == 0) {
                ?>
                <div class="alert alert-danger col-lg-offset-2 col-lg-8" role="alert">
                    <strong>Ouuuuups!</strong> Votre publication n'a pas pu être supprimée. Réessayez ultérieurement!
                </div>
            <?php }//Fin else if ==0
        }#Fin if isset
        ?>

        <?php if (isset($_GET['delrslt'])) {
            if ($_GET['delrslt'] == 1) {
                ?>
                <div class="alert alert-success" id="modrslt1" role="alert">
                    <strong>Suppression effectuée!</strong> Résultat supprimé avec succès.
                </div>
            <?php } else if ($_GET['delrslt'] == 0) {
                ?>
                <div class="alert alert-danger col-lg-offset-2 col-lg-8" id="modrslt0" role="alert">
                    <strong>Ouuuuups!</strong> Ce résultat n'a pas pu être supprimé. Réessayez ultérieurement!
                </div>
            <?php }//Fin else if ==0
        }#Fin if isset
        ?>
    ...
    <table class="row">
        ...
        <?php while ($donnees = $reponse_pub->fetch()) {//Affichage publication
            ?>
            <tr class="row">
                <td class="col-xs-1">

                    <div class="row">
                        <a href="supprimer_pub.php?idpub=<?php bye_XSS($donnees['id_publication']); ?>"
                           title="Supprimer publication <?php bye_XSS($donnees['id_publication']); ?>">
                            <div class="col-lg-1 col-md-1 col-xs-1">
                                <img src="../images/delete.png" width="10" length="10"/>
                            </div>
                        </a>
    ...
    <table class="row">
        ...
        <tr class="row">
            <td class="col-xs-1">
                <div class="row">
                    <a href="supprimer_result.php?idrslt=<?php bye_XSS($donnees['id_resultat']); ?>"
                       title="Supprimer résultat <?php bye_XSS($donnees['id_resultat']); ?>">
                        <div class="col-lg-1 col-md-1 col-xs-1">
                            <img src="../images/delete.png" width="10" length="10"/>
                        </div>
                    </a>
