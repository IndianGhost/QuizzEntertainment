      ...
      <div class="row">
        <div class="col-lg-offset-3 col-lg-6 col-xs-12">
          <form method="post" action="ajout_result.php" enctype="multipart/form-data">
            <fieldset>
              <legend>Modification du résultat <?php bye_XSS($donnees['id_resultat']);?> associé à la publication <?php bye_XSS($donnees['id_publication']);?></legend>
              <input type="hidden" name="id_resultat" value="<?php bye_XSS($donnees['id_resultat']);?>">
              <div class="row labeldiv">
                <label for="titre_resultat" class="col-xs-6">Titre du résultat :</label><input type="text" name="titre_resultat" id="titre_resultat" class="col-xs-5 col-xs-offset-1" value="<?php bye_XSS($donnees['titre_resultat']);?>" />
              </div>

              <div class="row labeldiv">
                <label for="description_resultat" class="col-xs-6">Description du résultat :</label><input type="text" name="description_resultat" id="description_resultat" class="col-xs-5 col-xs-offset-1"  value="<?php bye_XSS($donnees['description_resultat']);?>"/>
              </div>
...
?>
    ...
    <div class="row">
      <div class="col-lg-12">
        ...
        <p class="lead">Si vous désiriez modifier la publication associée au résultat, vous devriez resaisir à nouveau l'image correspondante. Sinon, vous risqueriez d'avoir des lacunes.</p>
        <br/>
        <p><strong>N.B : </strong>La taille des image ne doit pas dépasser 8Mo et leurs extensions doivent être (.jpg). Il faut que la résolution soit (de préférence) 717 x 475 afin d'atteindre un affichage parfait.</p>
      </div>
    </div>
